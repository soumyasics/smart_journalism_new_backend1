...
const login = (req, res) => {
  ...
  public.findOne({ email }).exec().then (user => {
...
const editPublicById=(req,res)=>{
  ...
  public.findByIdAndUpdate({_id:req.params.id},{
    name:req.body.name,
    email:req.body.email,
        age:req.body.age,
        gender:req.body.gender
...
const viewPublicById=(req,res)=>{
  public.findOne({_id:req.params.id}).exec()
  .then(data=>{
...
const forgotPassword=(req,res)=>{
    public.findOne({email:req.body.email}).exec()
    .then(data=>{
      if(data==null){
        res.json({
          status:500,
        ...
        public.findOneAndUpdate({email:req.body.email},{
          password:req.body.password
        }).exec().then(data=>{
          res.json({
            status:200,
  ...
  const deletePublicById=(req,res)=>{
    public.findByIdAndDelete({_id:req.params.id}).exec()
    .then(data=>{
...
const sendKey = (req, res) => {
  public.findById({ _id: req.params.id }).exec()
    .then(data => {
...
const verifyKey = (req, res) => {
  ...
  public.findById({ _id: req.params.id }).exec()
    .then(data => {
      ...
      if (key == key1) {
        public.findByIdAndUpdate({ _id: req.params.id }, { verified: true }).exec()
        .then(data1 => {
          res.json({
            status: 200,
            msg: "Data send successfully",
...
const deleteSavedNews=(req,res)=>{
  savedNewses.findByIdAndDelete({_id:req.params.id})
...
module.exports={registerPublic,login,viewPublicById,editPublicById,viewPublic
,deletePublicById,forgotPassword,sendKey,verifyKey,saveNews,
viewSavedNewsByPid,deleteSavedNews}